  ...
  useEffect(() => {
    const fetchStations = async () => {
      try {
        ...
        const response = await fetch(`https://wodyapi.nowaccy.cloud/api/hydrodata/stations/related?riverName=${encodeURIComponent(riverName)}`);
        if (!response.ok) {
        ...
        const detailedStations = await Promise.all(
          basicStations.map(async (station) => {
            try {
              const detailResponse = await fetch(`https://wodyapi.nowaccy.cloud/api/hydrodata/stations/${station.id}`);
    ...
    const fetchChartData = async () => {
      ...
      try {
        ...
        const chartResponse = await fetch(
          `https://wodyapi.nowaccy.cloud/api/hydrodata/stations/${selectedStation.id}/chart?startDate=${formatDate(startDate)}&endDate=${formatDate(endDate)}`
        );
